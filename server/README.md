...
| User accounts / login | Yes | Yes | OAuth / Local Auth |
...
The template supports **OAuth / Local Auth** for user authentication. It works differently on native and web:
...
2. `startOAuthLogin()` calls `Linking.openURL()` which opens OAuth login in the system browser
...
2. Browser redirects to OAuth portal
3. User authenticates
...
When you need external data, use the omni_search with search_type = 'api' to check available built-in APIs first. Only connect external APIs when no suitable built-in option exists.
...
| `BUILT_IN_FORGE_API_URL` | Built-in API endpoint |
