...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // If user not in DB:
    // - Local users (openId starts with 'local_') are self-contained, no OAuth sync needed
    // - OAuth users: try to sync from configured OAuth server
    if (!user) {
      if (sessionUserId.startsWith("local_")) {
        // Local registered user not found in DB (e.g., DB was wiped)
